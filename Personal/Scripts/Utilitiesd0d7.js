...
function validaNumeroOnInput(e) {
    e.value = e.value.replace(/[^0-9]/g, ''); e.value = e.value.replace(/(\..*)\./g, '$1');
    return e;
}
...
$(document).ready(function () {
    ...
    $('.SoloLetras').blockInput({ regex: '^[a-zA-Z áéíóúÁÉÍÓÚñÑ]+$' });
    $('.SoloNumerosLetras').blockInput({ regex: '^[a-zA-Z 0-9]+$' });
...
async function postData(url = '', data = {}, loading = false, ContentType = "application/json") {
    // Default options are marked with *
    if (loading) {
        //muestra el modal pleaseWaitDialog// debe estar en el html para que no mande error
        let myModal = new bootstrap.Modal(document.getElementById('pleaseWaitDialog'));
        myModal.show();
...
async function postDataNoAsync(url = '', data = {}, loading = false, ContentType = "application/json") {
    // Default options are marked with *
    if (loading) {
        //muestra el modal pleaseWaitDialog// debe estar en el html para que no mande error
        let myModal = new bootstrap.Modal(document.getElementById('pleaseWaitDialog'));
...
function ValidarCheckBox(idCheck, mensaje) {
    if ($("#" + idCheck).prop("checked") != true) { //valida autorizacion de datos
        $("#" + idCheck).focus();
        Swal.fire({ icon: '', title: '', text: mensaje });
        return false;
    } else {
        return true;
    }
...
function downloadPDF(base64, nombreArchivo = "archivo.pdf") {
    if (base64.toString() == "") { return false; }
